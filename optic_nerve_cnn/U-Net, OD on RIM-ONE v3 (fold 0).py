...
from optic_nerve_cnn.utils import get_unet_light_for_fold0, get_unet_light, log_dice_loss, mean_IOU_gpu, dice_metric, \
    tf_to_th_encoding, preprocess, folder, th_to_tf_encoding, dice
from scripts.dual_IDG import DualImageDataGenerator

K.set_image_dim_ordering('th')


def main(dataset='RIM_ONE_v3.hdf5', fold0=True):
    ...
    gen = data_generator(X, Y, 'train', batch_size=1, train_idx=train_idx, test_idx=test_idx)
    batch = next(gen)
    batch[0].shape
    ...
    history = model.fit_generator(data_generator(X, Y, train_or_test='train', batch_size=1),
                                  ...
                                  callbacks=[CSVLogger(os.path.join(folder(weights_folder), 'training_log.csv')),
                                             ReduceLROnPlateau(monitor='val_loss', mode='min', factor=0.5, verbose=1,
                                                               patience=40),
    ...
    show_img_pred_corr(best_idx, 'best', test_idx=test_idx, X=X, Y=Y, model=model)
    print('IOU: {}, Dice: {} (best)'.format(pred_iou[best_idx], pred_dice[best_idx]))
    show_img_pred_corr(worst_idx, 'worst', test_idx=test_idx, X=X, Y=Y, model=model)
...
def on_model_load(load_model, model):
    ...
    model_folder = os.path.join(os.path.dirname(os.getcwd()), 'models_weights',
                                '03.03,14_19,U-Net light, on RIM-ONE v3 256 px fold 0, SGD, high augm, CLAHE, log_dice loss')
...
def data_generator(X, y, train_idx, test_idx, train_or_test='train', batch_size=3, return_orig=False, stationary=False):
    while True:
...
def show_img_pred_corr(i, file_suffix, test_idx, X, Y, model):  # i is index of image in test_idx
    img_no = test_idx[i]
    ...
    plt.imsave('od_{dataset}_fold_0_{file_suffix}_case_image.png'.format(dataset=dataset, file_suffix=file_suffix),
               X[img_no])
    plt.imsave('od_{dataset}_fold_0_{file_suffix}_case_pred.png'.format(dataset=dataset, file_suffix=file_suffix), pred,
               cmap=plt.cm.Greys_r)
    plt.imsave('od_{dataset}_fold_0_{file_suffix}_case_corr.png'.format(dataset=dataset, file_suffix=file_suffix), corr,
               cmap=plt.cm.Greys_r)
