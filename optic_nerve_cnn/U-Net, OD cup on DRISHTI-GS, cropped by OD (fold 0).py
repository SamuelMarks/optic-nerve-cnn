...
def main(dataset='DRISHTI_GS', show_best_and_worst=True):
...
def before_train(X, h5f, dataset):
    ...
    train_idx = h5f['{dataset}/train_idx_cv'.format(dataset=dataset)][0]
    test_idx = h5f['{dataset}/test_idx_cv'.format(dataset=dataset)][0]
...
def on_model_load(load_model, model, dataset):
    ...
    model_folder = os.path.join(os.path.dirname(os.getcwd()), 'models_weights',
                                '02.03,13_57,OD Cup, U-Net light on {dataset} 512 px cropped to OD 128 px fold 0, SGD, log_dice loss'.format(
                                    dataset=dataset))
